	...
	<div class="my-5 p-5 pb-10 bg-white rounded-md">
		<div class="flex items-center">
			...
			<h3 class="text-gray-700 text-lg font-semibold ml-5">
				{{ comment.user.name }}
				<span class="text-gray-400 text-sm ml-2">{{ comment.timestamp }}</span>
			</h3>
		</div>
		...
		<form v-if="replyComment" @submit.prevent="createComment">
			<input id="comment_input" v-model="form.comment" type="text" class="w-full pl-5 pr-10 py-3 border border-gray-200 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm transition-colors" placeholder="Reply comment...">
		</form>
